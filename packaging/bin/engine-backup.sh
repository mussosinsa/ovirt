...
# config file.
export ENGINE_DEFAULTS="${ENGINE_DEFAULTS:-/usr/share/ovirt-engine/share/ovirt-engine/services/ovirt-engine/ovirt-engine.conf}"
export ENGINE_VARS="${ENGINE_VARS:-/usr/share/ovirt-engine/etc/ovirt-engine/engine.conf}"
export ENGINE_BACKUP_DEFAULT_DIR="${ENGINE_BACKUP_DEFAULT_DIR:-/usr/share/ovirt-engine/var/lib/ovirt-engine-backup}"
export ENGINE_BACKUP_LOG_DEFAULT_DIR="${ENGINE_BACKUP_LOG_DEFAULT_DIR:-/usr/share/ovirt-engine/var/log/ovirt-engine-backup}"
PACKAGE_NAME="ovirt-engine"
PACKAGE_VERSION="4.5.6"
DISPLAY_VERSION="4.5.6"
...
is_sourceable_config_file() {
	local conf="$1"
	[ -r "${conf}" ] || return 1
	bash -n "${conf}" >/dev/null 2>&1 || return 1
	return 0
}

source_config_file_if_possible() {
	local conf="$1"

	[ -e "${conf}" ] || return 0

	if ! is_sourceable_config_file "${conf}"; then
		decrypt_config_if_allowed "${conf}"
	fi

	if is_sourceable_config_file "${conf}"; then
		. "${conf}"
	else
		if [ "${conf}" = "${ENGINE_ETC}/engine.conf.d/10-setup-database.conf" ]; then
			die "Cannot parse required database configuration file: ${conf}"
		fi
		echo "WARN: Skipping unreadable/encrypted configuration file: ${conf}" >&2
	fi
}
...
load_config_dir() {
	for f in \
		...
		source_config_file_if_possible "${f}"
	done
}
...
_DECRYPT_ALLOWED_BASENAMES="internal.properties 10-setup-database.conf 10-setup-dwh-database.conf"
ENCRYPTOR_TOOL="/usr/share/ovirt-engine/encryptor/encryptor.py"
DECRYPT_RESTORE_FILES=""

register_decrypt_restore_file() {
	local original="$1"
	local restore="$2"
	DECRYPT_RESTORE_FILES="${DECRYPT_RESTORE_FILES}
${original}|${restore}"
}

restore_decrypted_configs() {
	local entry=""
	local original=""
	local restore=""

	[ -n "${DECRYPT_RESTORE_FILES}" ] || return 0
	...
	while IFS= read -r entry; do
		...
		[ -n "${restore}" ] || continue
		if [ -f "${restore}" ]; then
			cp -a "${restore}" "${original}" >/dev/null 2>&1 || true
			rm -f "${restore}" >/dev/null 2>&1 || true
		fi
	done << __DECRYPT_RESTORE_FILES__
${DECRYPT_RESTORE_FILES}
__DECRYPT_RESTORE_FILES__

	DECRYPT_RESTORE_FILES=""
}

decrypt_config_if_allowed() {
	local conf="$1"
	local base="${conf##*/}"
	local allowed=1
	local b

	for b in ${_DECRYPT_ALLOWED_BASENAMES}; do
		if [ "${base}" = "${b}" ]; then
			allowed=0
			break
		fi
	...
	[ ${allowed} -ne 0 ] && return 0
	[ -f "${conf}" ] || return 0
	[ -f "${ENCRYPTOR_TOOL}" ] || return 0
	...
	local restore=""
	restore="$(mktemp)" || return 0
	cp -a "${conf}" "${restore}" || {
		rm -f "${restore}"
		return 0
	}

	if "${python_bin}" "${ENCRYPTOR_TOOL}" --decrypt "${conf}" >/dev/null 2>&1; then
		register_decrypt_restore_file "${conf}" "${restore}"
		return 0
	fi
	cp -a "${restore}" "${conf}" >/dev/null 2>&1 || true

	if "${python_bin}" "${ENCRYPTOR_TOOL}" -d "${conf}" >/dev/null 2>&1; then
		register_decrypt_restore_file "${conf}" "${restore}"
		return 0
	fi
	cp -a "${restore}" "${conf}" >/dev/null 2>&1 || true
...
my_load_config() {
	...
	decrypt_config_if_allowed "/etc/ovirt-engine/aaa/internal.properties"
	decrypt_config_if_allowed "${ENGINE_ETC}/engine.conf.d/10-setup-database.conf"
	decrypt_config_if_allowed "${ENGINE_ETC}/engine.conf.d/10-setup-dwh-database.conf"
	decrypt_config_if_allowed "${DWH_CONFIG}.d/10-setup-database.conf"
	load_config_dir
...
getOSVersion() {
	python3 -c 'import distro; d = distro.linux_distribution(full_distribution_name=0); print("%s%s" % (d[0], d[1].split(".")[0]))'
}

looseVersion_le() {
	# return 0 if first arg is <= second arg
	python3 -c 'import sys; from distutils.version import LooseVersion; sys.exit(0 if LooseVersion(sys.argv[1]) <= LooseVersion(sys.argv[2]) else 1)' "$1" "$2"
}
