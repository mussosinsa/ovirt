...
# We have to call this here to ensure the requests package
# uses monkey patched objects
...
def connect_volume(args):
    ...
    _write_output(json.dumps(conn), args.correlation_id)
...
def storage_stats(args):
    ...
    _write_output(
        json.dumps(backend.stats(refresh=args.refresh)), args.correlation_id)
...
def get_connection_info(args):
    ...
    _write_output(json.dumps(conn.connection_info), args.correlation_id)
...
def create_snapshot(args):
    ...
    _write_output(snap.id, args.correlation_id)
...
def create_volume_from_snapshot(args):
    ...
    _write_output(new_vol.id, args.correlation_id)
...
def _write_output(s, correlation_id=None):
    prefix = correlation_id if correlation_id else "error"
    sys.stdout.write("{}:{}".format(prefix, s))
    sys.stdout.flush()
